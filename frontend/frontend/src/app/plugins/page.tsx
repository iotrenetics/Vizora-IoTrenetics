...
const PLUGINS = [
  ...
  { id: 'prometheus',  name: 'Prometheus',      author: 'IoTrenetics',   type: 'datasource', signed: true,  installed: false, href: '#',                 emoji: '🔥', color: '#f43f5e' },
  { id: 'postgresql',  name: 'PostgreSQL',      author: 'IoTrenetics',   type: 'datasource', signed: true,  installed: false, href: '#',                 emoji: '🐘', color: '#3b82f6' },
  { id: 'loki',        name: 'Loki',            author: 'IoTrenetics',   type: 'datasource', signed: true,  installed: false, href: '#',                 emoji: '📜', color: '#8b5cf6' },
  { id: 'timescale',   name: 'TimescaleDB',     author: 'Timescale Inc', type: 'datasource', signed: true,  installed: false, href: '#',                 emoji: '⏱',  color: '#f97316' },
...
export default function PluginsPage() {
  ...
  const toggleInstall = (id: string, e: React.MouseEvent) => {
    e.preventDefault();
    setInstalled(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  };
    ...
    <div style={{ width: '100%' }}>
      ...
      {!dismissed && (
        <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
          ...
          <button style={{ padding: '5px 14px', borderRadius: 6, border: '1px solid var(--accent)', background: 'transparent', color: 'var(--accent)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Syne, sans-serif', whiteSpace: 'nowrap' }}>
            → Go to Advisor
          </button>
        ...
        <div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search Vizora plugins"
              style={{ width: '100%', padding: '9px 10px 9px 32px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-primary)', fontSize: 13, outline: 'none', fontFamily: 'Syne, sans-serif', boxSizing: 'border-box' }} />
          </div>
        </div>
        <div>
          <p style={{ margin: '0 0 5px', fontSize: 12, fontWeight: 600, color: 'var(--text-muted)' }}>Type</p>
          <select value={typeFilter} onChange={e => setTypeFilter(e.target.value as TypeFilter)}
            style={{ padding: '9px 32px 9px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-primary)', fontSize: 13, outline: 'none', cursor: 'pointer', fontFamily: 'Syne, sans-serif', minWidth: 140 }}>
          ...
          <div style={{ display: 'flex', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden', background: 'var(--surface)' }}>
            {(['all', 'installed'] as StateFilter[]).map(s => (
              <button key={s} onClick={() => setStateFilter(s)}
                style={{ padding: '8px 16px', background: stateFilter === s ? 'var(--surface-2)' : 'transparent', border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: stateFilter === s ? 600 : 400, color: stateFilter === s ? 'var(--text-primary)' : 'var(--text-muted)', fontFamily: 'Syne, sans-serif', borderRight: s === 'all' ? '1px solid var(--border)' : 'none', transition: 'all 0.12s', textTransform: 'capitalize' }}>
                {s}
              </button>
            ))}
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
        {filtered.map((p, i) => {
          ...
          const isLink = p.href !== '#';
          const CardTag = isLink ? Link : 'div';
          const cardProps = isLink ? { href: p.href } : {};
          return (
            <motion.div key={p.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
              {/* @ts-ignore */}
              <CardTag {...cardProps} style={{ textDecoration: 'none', display: 'block' }}>
                <div style={{ padding: '18px 18px 14px', borderRadius: 12, border: `1px solid ${isInstalled ? 'rgba(99,102,241,0.3)' : 'var(--border)'}`, background: 'var(--surface)', cursor: 'pointer', transition: 'all 0.15s', height: '100%', boxSizing: 'border-box' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 16px rgba(0,0,0,0.07)'; (e.currentTarget as HTMLElement).style.borderColor = p.color + '60'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = 'none'; (e.currentTarget as HTMLElement).style.borderColor = isInstalled ? 'rgba(99,102,241,0.3)' : 'var(--border)'; }}>

                  {/* Top row: icon + name + type icon */}
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 44, height: 44, borderRadius: 10, background: p.color + '18', border: `1px solid ${p.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>
                        {p.emoji}
                      </div>
                      <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    <button onClick={e => toggleInstall(p.id, e)}
                      style={{ marginLeft: 'auto', padding: '4px 12px', borderRadius: 6, border: `1px solid ${isInstalled ? '#fca5a5' : 'var(--accent)'}`, background: isInstalled ? 'transparent' : 'var(--accent)', color: isInstalled ? '#dc2626' : '#fff', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'Syne, sans-serif', transition: 'all 0.12s' }}>
