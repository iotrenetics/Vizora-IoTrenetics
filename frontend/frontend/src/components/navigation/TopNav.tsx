...
function IconBtn({ children, onClick, title, active, badge }: {
  ...
  return (
    <button onClick={onClick} title={title}
      ...
      onMouseLeave={e => {
        (e.currentTarget as HTMLElement).style.background = active ? 'var(--surface-2)' : 'transparent';
        (e.currentTarget as HTMLElement).style.color = 'var(--text-secondary)';
        (e.currentTarget as HTMLElement).style.borderColor = active ? 'var(--border-2)' : 'transparent';
      }}
    >
      {children}
      {badge != null && badge > 0 && (
  ...
  return (
    <>
      <header style={{
        ...
        {/* ── Logo + Vizora name (shown when sidebar is collapsed) ── */}
        {collapsed && (
          <div style={{
            ...
            {TIME_PRESETS.map(t => (
              <button key={t.value} onClick={() => setSelectedTime(t.label)}
                style={{
                  ...
                  padding: '7px 12px',
                  background: selectedTime === t.label ? 'var(--accent-soft)' : 'transparent',
        ...
        {/* ── New ── */}
        <Dropdown width={200} align="right" trigger={
          <button style={{
            display: 'flex', alignItems: 'center', gap: 6,
          ...
          <div>
            ...
            <div style={{ maxHeight: 320, overflowY: 'auto' }}>
              {MOCK_ALERTS.slice(0, 6).map(alert => (
                <div key={alert.id}
                  style={{ display: 'flex', gap: 10, padding: '10px 14px', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'background 0.1s' }}
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--surface-2)'}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}
                >
                  <div style={{
                    width: 8, height: 8, borderRadius: '50%', marginTop: 4, flexShrink: 0,
                    background: alert.state === 'firing' ? 'var(--red)' : alert.state === 'pending' ? 'var(--amber)' : 'var(--green)',
                    ...(alert.state === 'firing' ? { animation: 'pulse-dot 2s infinite' } : {}),
                  }} />
        ...
        <Dropdown width={220} trigger={
          <div
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 6px 3px 3px', borderRadius: 8, cursor: 'pointer', transition: 'background 0.12s' }}
            onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = 'var(--surface-2)'}
            onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}
          >
            <div style={{
          ...
          <div>
            <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border)' }}>
              {/* Logo + name row in profile dropdown */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <VizoraMark size={20} />
                <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.04em' }}>VIZORA</span>
              </div>
              ...
              <div style={{
                marginTop: 6, display: 'inline-block', fontSize: 10, padding: '2px 8px',
